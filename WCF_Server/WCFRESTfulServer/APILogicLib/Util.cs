...
namespace APILogicLib
{
    public static class Util
    {
        ...
        public static DateTime yyyyMMddHHmmssToDateTime(string dateTime)
        {
        ...
        {
            string strCurDateTime = DateTimeToyyyyMMddHHmmss(DateTime.Now);
        ...
        {
            string strCurDateTime = DateTimeToyyyyMMddHHmmss(time);
            IFormatProvider culture = new System.Globalization.CultureInfo("", true);
        ...
        public static DateTime 문자열에서_시간타입_변환하기(string timeString)
        {
            DateTime date = DateTime.ParseExact(timeString, "yyyyMMddHHmm", null);
            return date;
        }

        public static double CheckPastDayFromNow(string date)
        {
            var now = DateTime.Now;
            var datetime = DateTime.ParseExact(date, "yyyyMMddHHmmss", null);

            now = DateTime.ParseExact(now.ToString("yyyyMMdd"), "yyyyMMdd", null);
            datetime = DateTime.ParseExact(datetime.ToString("yyyyMMdd"), "yyyyMMdd", null);

            TimeSpan result = now - datetime;

            return result.Days;

        }

        public static double CheckPastDayFromNow(DateTime date)
        {
            var now = DateTime.Now;

            now = DateTime.ParseExact(now.ToString("yyyyMMdd"), "yyyyMMdd", null);
            date = DateTime.ParseExact(date.ToString("yyyyMMdd"), "yyyyMMdd", null);
        ...
        public static double CheckPastDay(string startdate, string enddate)
        {
            var startdatetime = DateTime.ParseExact(startdate, "yyyyMMddHHmmss", null);
            var enddatetime = DateTime.ParseExact(enddate, "yyyyMMddHHmmss", null);

            TimeSpan result = enddatetime - startdatetime;


            return Math.Ceiling(result.TotalDays);
        }

        public static double GetPastMinute(DateTime destTime, DateTime targetTime)
        {
            return (destTime - targetTime).TotalMinutes;
        }

        public static double GetPastSecond(DateTime destTime, DateTime targetTime)
        {
            return (destTime - targetTime).TotalSeconds;
        }

        public static Int64 DayToTick(int day)
        {
            var tick = new TimeSpan(day, 0, 0, 0).Ticks;
            return tick;
        }

        public static Int64 HourToTick(int hour)
        {
            var tick = new TimeSpan(hour, 0, 0).Ticks;
            return tick;
        }

        public static Int64 MinToTick(int minute)
        {
            var tick = new TimeSpan(0, 0, minute, 0).Ticks;
            return tick;
        }

        public static Int64 SecondToTick(int second)
        {
            var tick = new TimeSpan(0, 0, 0, second).Ticks;
            return tick;
        }

        public static bool 지정한_기간을_넘었다(Int64 checkTick, Int64 curTick, Int64 기간Tick)
        {
        ...
        public static string MakePerfactID(string str)
        {
            string perfact = "";
            for (int i = 0; i < str.Length; ++i)
            {
                if (IsEnglish(str[i]) == true || IsNumeric(str[i]) == true || str[i] == '-')
                {
                    perfact += str[i];
                }
            }
        ...
        public static bool IsKoren(char ch)
        {
